...
class PantallaCategorias extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final presupuestoAsync = ref.watch(presupuestoCategoriasProvider);

    // Mientras cargue cualquiera de los dos, mostramos loading
    if (categoriasAsync.isLoading || presupuestoAsync.isLoading) {
      return Scaffold(
        appBar: AppBar(
    ...
    final categorias = categoriasAsync.valueOrNull ?? [];
    final usarPresupuestos = presupuestoAsync.valueOrNull ?? false;
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () => showDialog(
          context: context,
          builder: (_) => _DialogoCategoria(usarPresupuestos: usarPresupuestos),
        ),
      ),
      body: categorias.isEmpty
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, index) => _CategoriaTile(
                categoria: categorias[index],
                usarPresupuestos: usarPresupuestos,
              ),
            ),
    );
  }
...
class _CategoriaTile extends ConsumerWidget {
  ...
  final bool usarPresupuestos;

  const _CategoriaTile({
    required this.categoria,
    required this.usarPresupuestos,
  });
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final presupuesto = categoria.presupuesto;

    // FIX: Subtitle condicional
    Widget? subtitle;
    if (usarPresupuestos && presupuesto > 0) {
      subtitle = Text(
        'Presupuesto: \$${_formatear(presupuesto)}',
        style: const TextStyle(color: Colors.green),
      );
    } else if (usarPresupuestos) {
      subtitle = const Text(
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            // FIX: Campo de presupuesto SOLO si está activado
            if (widget.usarPresupuestos) ...[
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: () async {
            ...
            final presupuesto = widget.usarPresupuestos
                ? (double.tryParse(_presuController.text.replaceAll('.', '')) ?? 0.0)
                : 0.0;
            ...
            if (esNueva) {
              await notifier.agregar(CategoriaModel(
                nombre: nombre,
                icono: icono,
